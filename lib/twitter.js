...
Twitter.prototype.post = function(url, content, content_type, callback) {
  ...
  console.log('POSTing request to Twitter stream:' +
    require('util').inspect(reqOpts, false, null));
    ...
    if(response.statusCode > 200) {
      ...
      var fun = function(arg) {
        if (arg != null && typeof arg != 'undefined') {
          util.debug('Site Stream API error response: ' + arg.toString());
        }
        util.debug(util.inspect(arguments, false, null));
      };
      response.on('data', fun);
      response.on('error', fun);
      response.on('end', fun);
      setTimeout(function() {
        /////
        // Original lines of code
        stream.destroySilent();
        stream.emit('error', 'http', response.statusCode, response);
        /////
      }, 1500);
      /////
 ...
 */
Twitter.prototype.addUsersToSiteStream = function(controlUri, userIds, callback) {
  // The controlUri given to the user by the Site Stream start with "/2b", but
  // ntwitter's keys.js file has a base Site Stream URL that ends with "/2b"
  var base = this.options.site_stream_base.replace(/\/2b$/, '')
  var url = base + controlUri + '/add_user.json';
  var form = 'user_id=' + userIds.join(',');
  console.log("addUsersToSiteStream url: " + url + ' ' + form);
  this.post(url, form, null, callback);
  return this;
}
